...
function ChatInputGIFList(props: IProps) {
  ...
  // 用于输入框的防抖
  const { run: search, loading: searchLoading } = useRequest(getGIFData, {
    debounceWait: 700,
    manual: true,
  })

  useEffect(() => {
    if (showGIF && gifList.length === 0) {
      run()
    }
  }, [showGIF])
    ...
    <AnimationBox show={showGIF || false}>
      <div className="pb-2 flex flex-col gap-2">
        ...
        <InfiniteList<GIFDataListItem>
          ...
          data={gifList}
          reloading={loading || searchLoading}
          isError={isError}
          isEmpty={isEmpty}
          ...
          renderAbnormalEmpty={() => (
            <div className="w-full h-full flex flex-col items-center justify-center gap-2">
              ...
              <Button size="medium" onClick={() => run(keyword)}>
                Retry
              </Button>
            </div>
          )}
