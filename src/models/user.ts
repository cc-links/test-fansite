...
import { STORAGE_TOKEN_KEY } from '@/constants/env'
...
export function getIsTourist() {
  const userInfo = useUserStore.getState().userInfo
  return userInfo?.loginMode === EnumLoginMode.Guest
}
...
export function changeUserInfoScene(data: Record<string, boolean>) {
  const userInfo = useUserStore.getState().userInfo
  const { sceneOldUser = {} } = userInfo || {}
  useUserStore.setState({
    // @ts-expect-error 这里需要修改类型
    userInfo: {
      ...userInfo,
      sceneOldUser: {
        ...sceneOldUser,
...
export function initUserInfo(userInfo: IUserInfo | null) {
  ...
  if (userInfo?.sessionId) {
    localStorage.setItem(STORAGE_TOKEN_KEY, userInfo.sessionId)
  }
}
...
/** 登入成功事件 */
export function ON_APP_EVENT_LOGIN_SUCCESS(res: IUserInfo) {
  // 触发登入成功
  _triggerLogined()
  ON_APP_EVENT_INIT()
  initUserInfo(res)
