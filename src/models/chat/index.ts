...
export async function chatListHandleNewMessage(p: XImChatMessage) {
  ...
  if (tId !== getActiveContactId()) {
    ...
    if (!updateTime || (updateTime && updateTime < p.unreadCount?.updateTime)) {
      setTotalUnreadCount(fId, p?.unreadCount?.totalUnreadCountData?.totalUnreadCount || 0)
    }
  }
...
// 更新输入中状态
export const updateContactIsTyping = (userId: string, toUserId: string, isTyping: boolean) => {
  const chatCacheManager = getOrCreateChatItemCacheManager(userId, toUserId)
  chatCacheManager.setChatItemDetails({
    isTyping,
  })
...
// 输入中上报
// 4秒的节流
const throttledTypingReport = throttle(callMessageTypingApi, 4 * 1000, {
  leading: true,
  trailing: true,
})
export const handleTypingReport = (toUserId: string) => {
  throttledTypingReport({
    toUserId,
  })
}
