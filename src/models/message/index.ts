...
import { updateContactIsTyping } from '../chat'
...
export const sendMessage = async (message: Partial<XImChatMessageData>) => {
  ...
  const newMessage = {
    ...
    price: message.price && message.price * 100,
 ...
 * 消息列表接受到新消息
 */
const debounceMessageRead = debounce(callMessageReadApi, 2 * 1000, {
  leading: true,
  trailing: true,
})
export const handleWsReceiveNewMessage = async (message: Partial<XImChatMessageData>, type?: XImMessageType) => {
  ...
  // 更新联系人正在输入状态
  updateContactIsTyping(fId, tId, false)
