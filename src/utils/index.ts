 ...
 * @returns void
 */
export const handlePriceChange = (value: string, callback: (value: string) => void): void => {
  // 只允许输入数字和小数点
  if (!/^\d*\.?\d*$/.test(value)) {
    return
  }
  ...
  if (value.includes('.')) {
    ...
    if (decimal.length <= 2) {
      ...
      callback(`${integer}.${decimal.slice(0, 2)}`)
    }
  } else {
    callback(value)
  }
 ...
 * @returns void
 */
export const handlePriceBlur = (value: string, callback: (value: string) => void): void => {
  if (!value) {
    callback('0.00')
    return
  }
