 ...
 * 获取名称的显示
 */
export const getShowText = (contactMeta: Partial<IContactMeta>) => {
  return (
    contactMeta?.user?.remarkName || contactMeta?.user?.nickName || contactMeta?.user?.userName || contactMeta?.userId
  )
}
