...
import { ChatInputSendServerData, EnumLoginMode } from '@/types'
...
export async function callSendMessageApi(params: ChatInputSendServerData): Promise<XImChatMessageData> {
  ...
  const isGuest = userInfo?.loginMode === EnumLoginMode.Guest
  const url = isGuest ? '/api/chat/message/send/anonymous' : '/api/chat/message/send'
...
// 获取hooks信息并发送hook消息
export const getHooksInfoAndSendHookMessage = async (params: { creatorId: string; hookId: string }) => {
  return request.post('/api/chat/hook', params)
}
