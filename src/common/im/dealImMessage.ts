...
export async function ON_IM_MESSAGE(message: XImMessage, isSync = false) {
  ...
  if (message.type === XImMessageType.MessageWithdraw) {
    ...
    if (getActiveContactId() !== message.data.toUserId) {
      // 更新总未读数
      setTotalUnreadCount(message.data.userId, message?.unreadCount?.totalUnreadCountData?.totalUnreadCount || 0)
      // 更新会话的未读数
  ...
  // 输入中的状态
  if (message.type === XImMessageType.Typing) {
    const { userId, toUserId } = message?.data ?? {}
    if (userId && toUserId) {
      updateContactIsTyping(userId, toUserId, true)
    }
  }
